...
大部分人对这一块应该没多大兴趣，我就简单带过，编译器主要会进行**词法分析(lexical analysis)**以及**语法分析(syntactic analysis)**，遍历语法树生成流，最后生成虚拟机的机器码，也就是字节码。这里的每一点都包含很大的信息量，就不再详细叙述编译的细节了。下面解析一下 Python 的执行，那就要牵扯到虚拟机和字节码了。
