...
面试中90%的算法题都从**leetcode hot100**和**剑指offer中出** 刷两遍非常有必要 （不对劲？）

第一遍刷的时候自己先想十分钟，如果没有思路直接看题解。
...
* 指针是一个新的变量，指向**另一个变量的地址**，我们可以通过访问这个地址来修改另一个变量；
* 引用是一个**别名**，对引用的操作就是对变量的本身进行操作
* 传参的时候，使用指针的话需要**解引用**才能对参数进行修改，而使用引用可以直接对参数进行修改
* 指针可以有多级，引用只有一级
* 指针可以不被初始化、引用必须被初始化。
* 指针可以被多次赋值指向不同的对象，而引用只能被绑定一次 (初始化指向的对象)。 
* 指针可以为空，引用不可以。
...
【note】静态成员函数要访问非静态成员时，要用过对象来引用。

```cpp
class Dummy{
public:
    int id; 
    Dummy(int _id) : id(_id) {}
    static void say(Dummy* d){
        std::cout << "Hello from " << d->id << std::endl;  
    }
};
...
注意到**成员函数**、**虚函数表**是所有**类的对象共享**的

**成员函数还是以一般的函数一样的存在。`a.fun()` 是通过 `fun(a.this)` 来调用的。**
...
* 算法包括排序，复制等常用算法，以及不同容器特定的**算法**。
* 容器就是数据的存放形式，包括***序列式容器***和***关联式容器***，
  * 序列式容器就是list，vector等，关联式容器就是set，map等。
* 迭代器就是在不暴露容器内部结构的情况下**对容器的遍历**。

#### （16） STL源码中的hash表的实现

STL中的hash表就unordered_map。使用的是哈希进行实现（注意与map的区别）。它记录的键是元素的哈希值，通过对比元素的哈希值来确定元素的值。

unordered_map的底层实现是hashtable，采用**开链法**（也就是用桶）来解决哈希冲突，当桶的大小超过8时，就自动转为**红黑树**进行组织。
...
1. **线性探查**。
   1. 假设哈希计算出的位置 $$H$$, 那么尝试 $$H+1, H+2, H+3... H+i$$
   2. 该元素的哈希值对应的桶不能存放元素时，**循序往后一一查找**， 直到找到一个空桶为止，
   3. 在查找时也一样，当哈希值对应位置上的元素与所要寻找的元素不同时，就往后一一查找，直到找到吻合的元素，或者空桶。
2. **二次探查**。
   1. 该元素的哈希值对应的桶不能存放元素时，
   2. 假设哈希计算出的位置 $$H$$, 那么尝试 $$H+1^2, H+2^2, H+3^2... H+i^2$$ 
   3. 解决了 Primaary Clustering 
3. **双散列函数法** Double Hashing 
   1. 当第一个散列函数发生冲突的时候 (初探位置 $$H_1 = h(k)$$)，
   2. 使用第二个散列函数进行哈希，作为**步长/偏移量** (即 $$H_1 + H_2$$ )。
   3. 用两种不同的 Hashing function 依赖于 key $$k$$ 

**开链法** : 发生冲突的地方，可以用另一种容器储存（例如**链表**、**红黑树**）

1. **开链法**。
   1. 在每一个桶中维护一个链表，由元素哈希值寻找到这个桶，然后将元素插入到对应的链表中，STL的hashtable就是采用这种实现方式。
2. 建立公共溢出区。
   1. 当发生冲突时，将所有冲突的数据放在公共溢出区。
...
list是由双向链表实现的，因此内存空间是不连续的。只能通过指针访问数据，**所以list的随机存取非常没有效率**，时间复杂度为o(n); 但由于链表的特点，能高效地进行插入和删除。（***内存的随机访问***）

**vector**拥有一段连续的内存空间，**能很好的支持随机存取**，因此**vector::iterator支持“+”，“+=”，“<”等操作符**。

**list**的内存空间可以是不连续，**它不支持随机访问**，因此**list::iterator则不支持“+”、“+=”、“<”等**

**vector::iterator和list::iterator都重载了“\+\+”运算符。** （**上面的符号都是随机访问**）
...
* 重载（overload）是指函数名相同，**参数列表不同**的函数实现方法。它们的返回值可以不同，但返回值不可以作为区分不同重载函数的标志。
* 重写（overwide）是**指函数名相同，参数列表相同，只有方法体不相同的实现方法。**
  * **一般用于子类继承父类时对父类方法的重写** (虚函数) 。子类的同名方法屏蔽了父类方法的现象称为隐藏。
...
C++ 多态包括**编译时多态**和**运行时多态**，

- **编译时多态**体现在**函数重载**和**模板(范型)**上，(一个函数可以有多种行为)
- **运行时多态**体现在**虚函数**上。 （类的成员函数可以有多种行为）
...
C++的虚函数是实现多态的机制。它是通过虚函数表实现的，**虚函数表是每个类中存放虚函数地址的指针数组**，

类的实例在调用函数时会在虚函数表中寻找函数地址进行调用，

一个类的所有实例都**共享同一张虚函数表**。[C++ 虚函数表解析 (ran.moe)](https://ran.moe/2020/12/21/cpp-virtual-function.html)

如果子类覆盖了父类的函数，则**子类的虚函数表会指向子类实现的函数地址，否则指向父类的函数地址**。

- 如果子类覆盖了父类函数，那么我想要**只表现子类的函数地址**
- 如果没有，我希望**继承**发挥作用，所以**子类指向父类的函数地址**。
- 实际上**父类和子类都有虚函数的映射**。不然没有办法实现多态。

详见：[C++虚函数表剖析](https://blog.csdn.net/lihao21/article/details/50688337)

* 如果多重继承和多继承的话，类的虚函数表长什么样子？
...
- 如果**类中有虚函数**，就将**虚函数的地址记录在类的虚函数表中**。
- **派生类在继承基类的时候，如果有重写基类的虚函数，就将虚函数表中相应的函数指针设置为派生类的函数地址，否则指向基类的函数地址。**
...
(一句话：虚函数表地址的指针需要存到对象里面。**在调用构造函数时，对象还不存在**。)
...
**静态绑定**也就是将该对象相关的属性或函数绑定为它的静态类型，也就是它在声明的类型，**在编译的时候就确定**。在调用的时候编译器会寻找它声明的类型进行访问。

**动态绑定**就是将该对象相关的属性或函数绑定为它的动态类型，**具体的属性或函数在运行期确定，通常通过虚函数实现动态绑定。**（因为**虚函数表**跟**内存地址**有关，而内存地址是运行时才能确定）。
...
**对象复用指得是设计模式**，对象可以采用不同的设计模式达到复用的目的，最常见的就是**继承**和**组合模式**了。
...
* `unique_ptr`是`auto_ptr`的一个改良版，不能赋值也不能拷贝，保证一个对象同一时间只有一个智能指针。而且 `unique_ptr` 是 scope pointer 
  * 只要不去写 Copy Constructor 和 Assignment Copy Constructor 就可以了。至于你说 raw point 。智能指针本来就不想你去碰 raw point 的。。。。
  * 但可以写 Move Semeatics (把 A 全部给 B， 此时 A 是 Rvalue (临时变量))
* `shared_ptr`可以使得一个对象可以有多个智能指针，当这个对象所有的智能指针被销毁时就会自动进行回收。（内部使用计数机制进行维护）
  * 不能写 Move Semeatics (赋值 B = A 的时候，把原来的 A 清空了，不道德啊，说好 A, B share 呢)
* `weak_ptr`是**为了协助`shared_ptr`而出现的**。它不能访问对象，只能观测`shared_ptr`的引用计数 (不会增加引用计数)，**防止出现相互引用死锁** (Java 的弱引用，否则垃圾回收也会出现问题)。
...
**编译器会在编译阶段将代码体嵌入内联函数的调用语句块中。** (**直接把函数调用替换掉**)
...
在声明的地方对模板代码本身进行编译，这次编译只会进行一个**语法检查**，并不会生成具体的代码。

在运行时**对代码进行参数替换后再进行编译**，生成具体的函数代码。
...
因为使用成员初始化列表进行初始化的话，**会直接使用传入参数的拷贝构造函数进行初始化**，省去了一次执行**传入参数的默认构造函数的过程**，否则会调用一次传入参数的默认构造函数 (否则需要两次构造函数)。所以使用成员初始化列表效率会高一些。<br>
...
* 1）static_cast ：
	用于各种**隐式转换**。具体的说，就是用户各种基本数据类型之间的转换，比如把int换成char，float换成int等。以及派生类（子类）的指针转换成基类（父类）指针的转换。

	>特性与要点：
	>
	>1. 它没有运行时类型检查，所以是有安全隐患的。
	>2. 在派生类指针转换到基类指针时，是没有任何问题的，在基类指针转换到派生类指针的时候，会有安全问题。(涉及到类指针的转换，应该用 dynamic_cast )
	>3. static_cast不能转换const，volatile等属性

* 2）dynamic_cast：
	用于动态类型转换。具体的说，就是***在基类指针到派生类指针***，或者***派生类到基类指针的转换***。
...
set,map的插入复杂度就是**红黑树的插入复杂度**，是log(N)。
...
* **定义**就是对这个**变量**和**函数**进行**内存分配**和**初始化**。
  * 需要分配空间，同一个变量可以被声明多次，但是只能被定义一次

#### （52）typdef和define区别

`#define` 是预处理命令，在预处理是执行简单的替换，不做正确性的检查

`typedef` 是在**编译**时处理的，它是在自己的作用域内给已经存在的类型一个别名

#### （53）被free回收的内存是立即返还给操作系统吗？为什么

https://blog.csdn.net/YMY_mine/article/details/81180168

不是的，**被free回收的内存会首先被ptmalloc使用双链表保存起来**，当用户下一次申请内存的时候，会尝试从这些内存中**寻找合适的返回**。这样就避免了频繁的系统调用，占用过多的系统资源。**同时ptmalloc也会尝试对小块内存进行合并，避免过多的内存碎片。**

[为什么malloc时需要指定size，对应的free时不需要指定size - Youmai の Blog (michaelyou.github.io)](http://michaelyou.github.io/2015/03/20/为什么malloc时需要指定size，对应的free时不需要指定size/)
...
根据 `control_block` 就可以知道被分配的内存大小。
...
友元提供了不同类的成员函数之间、类的成员函数和一般函数之间进行**数据共享的机制**。通过友元，一个不同函数或者另一个类中的成员函数可以访问类中的私有成员和保护成员。友元的正确使用能提高程序的运行效率，但同时也破坏了类的封装性和数据的隐藏性，导致程序可维护性变差。
...
使用场景是 **“并发”** 。比如两个进程用 Shared Memory 实现数据共享。**此时你必须每次都从内存中读取这个变量**， 如果编译器优化了，就造成 Cache 和 内存的内容不一样。


#### （57） STL中的sort()算法是用什么实现的，stable_sort()呢

STL中的sort是用**快速排序**和**插入排序**结合的方式实现的，**stable_sort()是归并排序**。

[C++一道深坑面试题：STL里sort算法用的是什么排序算法？ - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/36274119)

<img src="/shared/imgs/v2-8fa032e195365f77fb6b980a4ed71958_1440w.jpg" alt="img" style="zoom:33%;" />
...
#### （2） 继上一题，说明socket网络编程有哪些系统调用？其中close是一次就能直接关闭的-中呢包含了这些字段，标志位SYN和ACK都为1，而小ack为i+1，此时位于SYN-RCVD状态，这是第二次握手
...
<img src="/shared/imgs/image-20210629235054955.png" alt="image-20210629235054955" style="zoom:50%;" />
...
![TCP 拥塞控制算法- 知乎](/shared/imgs/v2-9d28c9a94501968e710b158ad6c4748c_1440w.jpg)
...
* 管道：管道是半双工的，双方需要通信的时候，需要建立两个管道。管道的实质是一个内核缓冲区，进程以先进先出的方式从缓冲区存取数据：管道一端的进程顺序地将进程数据写入缓冲区，另一端的进程则顺序地读取数据，该缓冲区可以看做一个循环队列，读和写的位置都是自动增加的，一个数据只能被读一次，读出以后再缓冲区都不复存在了。当缓冲区读空或者写满时，有一定的规则控制相应的读进程或写进程是否进入等待队列，当空的缓冲区有新数据写入或慢的缓冲区有数据读出时，就唤醒等待队列中的进程继续读写。管道是最容易实现的
	...
	<img src="/shared/imgs/2012110216160766.jpg" alt="img" style="zoom:50%;" />
...
* 三态模型
	...
	3. 阻塞：有IO事件或者等待其他资源
		<img src="/shared/imgs/20201230183459383.png" style="zoom:50%;" />
* 五态模型
	1. 新建态：进程刚刚创建。
	2. 就绪态：
	3. 运行态：
	4. 等待态：出现等待事件
	5. 终止态：进程结束
		<img src="/shared/imgs/20201230184726237.png" alt="img" style="zoom:50%;" />
...
* 七态模型
	...
	6. 挂起等待态
	7. 终止态
		<img src="/shared/imgs/20200629161028725.png" alt="img" style="zoom:50%;" />
...
我们可以使用外部排序来对它进行处理。首先将整个文件分成许多份，比如说m份，划分的依据就是使得每一份的大小都能放到内存里。然后我们用快速排序或者堆排序等方法对每一份数据进行一个内部排序，变成有序子串。接着对这m份有序子串进行**m路归并排序**。取这m份数据的最小元素，进行排序，输出排序后最小的元素到结果中，同时从该元素所在子串中读入一个元素，直到所有数据都被输出到结果中为止。
...
* 粗查找和细查找 (每隔10层扔一次， 缩小范围后。破了就可以缩小到10层内，然后每隔一层扔一次)
* 动态规划 ()
...
#### （13）掰巧克力问题 N*M块巧克力，每次掰一块的一行或一列，掰成1*1的巧克力需要多少次？（1000个人参加辩论赛，1V1，输了就退出，需要安排多少场比赛）
...
* 在项目中曾经遇到了**新的框架**不知道该如何上手的问题，以及**面对新的概念**，**新的技术不知道从何学起**。
	* 解决的办法是在**官网寻找说明文档和demo**，按照说明文档上的内容一步步了解，**以及咨询身边有用过这个框架的同学**，**或者在CSDN上寻找相关博客**。
* 项目的时间比较紧迫，没有那么多的时间可以用。解决方法是把还没有完成的项目**分一个轻重缓急**，**在有限的时间里，先做重要而且紧急的，然后完成紧急的，再做重要的。**利用轻重缓急做一个取舍。
...
- 我觉得 任何一家单位都有可能要加班。如果自己的工作没有按时完成，那自觉加班是理所当然的，当然，自己要不断提高工作效率，避免这种原因导致的加班。
- 合理规划、安排时间、提高工作效率来避免加班

- 如果遇到紧急任务或者突发状况时，为了顺利配合团队完成任务，我会尽自己所能加班共同完成。
